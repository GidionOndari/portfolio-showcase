...
function SafeImage({ src, alt, placeholder, className = '' }) { const [ok, setOk] = useState(true); return ok ? <img src={src} alt={alt} className={className} loading="lazy" onError={() => setOk(false)} /> : <div className="image-fallback">{placeholder}</div> }

export default function App(){return <div className="site-shell">
  <header className="hero card"><p className="pill">Small business website prototype</p><p className="eyebrow">PureGlow Naturals</p><h1>Premium handmade soaps and skincare essentials</h1><p className="subtitle">Luxury-style presentation for modern beauty and business credibility.</p><div className="cta-row"><a href="#products" className="btn btn-primary">View Products</a><a href="https://wa.me/254742917779" className="btn btn-outline" target="_blank" rel="noreferrer">Order on WhatsApp</a></div></header>
